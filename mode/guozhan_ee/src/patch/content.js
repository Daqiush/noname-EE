...
export const chooseCharacterContent = async (event, _trigger, _player) => {
	...
	for (const character in lib.character) {
		...
		if (get.config("onlyguozhan")) {
			if (!lib.characterGuozhanEEFilter.some(pack => lib.characterPack[pack]?.[character])) {
				continue;
			}
	...
	function createCharacterDialog() {
		const dialogxx = ui.create.characterDialog(
			...
			function (i) {
				...
				if (get.config("onlyguozhan")) {
					if (!lib.characterGuozhanEEFilter.some(pack => lib.characterPack[pack]?.[i])) {
						return true;
					}
					if (get.is.jun(i)) {
						return true;
			...
			get.config("onlyguozhan") ? "onlypack:mode_guozhan_ee" : undefined
		);
