...
(function () {
	...
	var messages = {
		...
		enter: function (key, nickname, avatar) {
			console.log("[server.enter] key="+key+" rooms="+rooms.length);
			this.nickname = util.getNickname(nickname);
			this.avatar = avatar;
			var room = false;
			for (var i of rooms) {
			...
			if (!room) {
				console.log("[server.enter] room not found, sending enterroomfailed");
			...
			if (room.owner) {
				if (
					...
					console.log("[server.enter] enterroomfailed: config="+JSON.stringify(room.config));
					this.sendl("enterroomfailed");
				} else {
					console.log("[server.enter] success, sending onconnection wsid="+this.wsid);
					this.owner = room.owner;
					this.owner.sendl("onconnection", this.wsid);
				}
				util.updaterooms();
			} else {
				console.log("[server.enter] room has no owner");
			}
		},
